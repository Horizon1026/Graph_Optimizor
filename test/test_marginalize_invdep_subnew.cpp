...
int main() {
    ...
    using Matrix3 = GraphOptimizor::Matrix3<Scalar>;
    using Vector3 = GraphOptimizor::Vector3<Scalar>;
    using Quaternion = GraphOptimizor::Quaternion<Scalar>;
    using MatrixX = GraphOptimizor::MatrixX<Scalar>;
    using VectorX = GraphOptimizor::VectorX<Scalar>;
    ...
    // 第三步：构造第0、1、2帧相机 pose 节点，并添加到 problem 中
    ...
    for (size_t i = 0; i < cameras.size() - 1; ++i) {
        // 给相机位置和姿态初值增加噪声
        Matrix3 noise_R, noise_X, noise_Y, noise_Z;
        noise_X = Eigen::AngleAxis<Scalar>(camera_rotation_noise(generator), Vector3::UnitX());
        noise_Y = Eigen::AngleAxis<Scalar>(camera_rotation_noise(generator), Vector3::UnitY());
        noise_Z = Eigen::AngleAxis<Scalar>(camera_rotation_noise(generator), Vector3::UnitZ());
        noise_R = noise_X * noise_Y * noise_Z;
        Vector3 noise_t(camera_position_noise(generator), camera_position_noise(generator), camera_position_noise(generator));
        Quaternion noise_q_wc(noise_R);
        Quaternion temp_q_wc = cameras[i].qwc * noise_q_wc;
        Vector3 temp_t_wc = cameras[i].twc + noise_t;
    ...
    // 第四步：构造在0、1、2帧观测的特征点 invdep 节点，同时构造 reprojection 边，添加到 problem 中
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        for (size_t j = 1; j < cameras.size() - 1; ++j) {
    ...
    // 第五步：配置相关参数，边缘化最旧帧
    std::cout << "\nStep 5: Start marginalization oldest to create prior." << std::endl;
    ...
    priorSize -= cameraVertices.front()->GetCalculationDimension();
    problem.Marginalize(needMarg, priorSize);
    MatrixX prior_H, prior_JTinv;
    VectorX prior_b, prior_r;
    ...
    // 第六步：基于边缘化最旧帧得到的先验信息，在帧1、2、3中边缘化次新帧
    std::cout << "\nStep 6: Create new problem to marg subnew." << std::endl;
    GraphOptimizor::Problem<Scalar> problem2;
    priorSize = 0;

    // 第七步：构造第1、2、3帧相机 pose 节点，并添加到 problem 中
    ...
    for (size_t i = 1; i < cameras.size(); ++i) {
        ...
        cameraVertex->SetType(type_cameraVertex);
        cameraVertices.emplace_back(cameraVertex);
        problem2.AddVertex(cameraVertex);
        priorSize += cameraVertex->GetCalculationDimension();
    }
    std::cout << "Add " << cameraVertices.size() << " camera vertices, problem2 has " << problem2.GetVertexNum() << " vertices." << std::endl;

    // 第八步：添加先验信息
    std::cout << "\nStep 8: Add prior information." << std::endl;
    problem2.SetPrior(prior_H, prior_b, prior_JTinv, prior_r);

    // 第九步：配置相关参数，边缘化最旧帧
    std::cout << "\nStep 9: Start marginalization." << std::endl;
    priorSize -= 6;
    problem2.SetMargnedVertexTypesWhenSolving(type_landmarkVertex);  // 设置在求解过程中需要暂时被边缘化的节点的类型
    needMarg.clear();
    needMarg.emplace_back(cameraVertices[cameraVertices.size() - 2]);
    problem2.Marginalize(needMarg, priorSize);

    // 第十步：打印出先验信息
    std::cout << "\nStep 10: Show prior result." << std::endl;
    problem2.GetPrior(prior_H, prior_b, prior_JTinv, prior_r);
    std::cout << "prior H is\n" << prior_H << std::endl;
    std::cout << "prior b is\n" << prior_b << std::endl;
    std::cout << "prior JTinv is\n" << prior_JTinv << std::endl;
    std::cout << "prior r is\n" << prior_r << std::endl;
